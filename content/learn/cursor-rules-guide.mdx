...
That is the same routing problem SKILL.md files have in Claude Code and Copilot, and it is where hand-written rules usually fall down: the content is fine, the description was written as a label rather than as a trigger. [SkillDraft's generator](/generate) writes for Cursor's `.mdc` format directly and scores the description density, which is quicker than tuning it by trial and error.
